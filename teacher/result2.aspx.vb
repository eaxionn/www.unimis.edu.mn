...
#End Region
        Public cnt1 As Integer = 1
        Public cnt As Integer = 1
        Public total As Double
        Public mstrSortCol As String
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            myConnection.ConnectionString = ConfigurationManager.AppSettings("ConnectionString")
            myConn.ConnectionString = ConfigurationManager.AppSettings("ConnectionString2")
        ...
        Sub setDropDown()
            Dim sql As String
            If myConnection.State = ConnectionState.Closed Then myConnection.Open()
            sql = "select SSS1='1',SSS2='нийт' union " & _
                "select SSS1,SSS2 from U0362 " & _
                "left join U021 on U021EE=U0362AA " & _
                "left join SUR_L on SSS1=U021BB " & _
                "where U0362DD='" & Request("lesscode") & "' and U0362EE='" & Session("tcode") & "' " & _
                "group by SSS1,SSS2 order by SSS1 "
        ...
        Function erhhaah() As Boolean
            ...
            Dim cmd = New SqlCommand("select isnull(U31CC,1) from U31 where U31AA='" & Session("tcode") & "'", myConnection)
            Dim dr = cmd.ExecuteReader()
            If dr.Read() Then
                If dr(0) = 0 Then erhhaah = False Else erhhaah = True
            End If
            dr.Close()
            myConnection.Close()
        End Function
        ...
        Sub Oyutniijagsaalt()
            ...
            cmd = New SqlDataAdapter("exec Dungiinlist1 @lesscode,@tcode,@ltype," & Label2.Text, myConnection)
        ...
        Sub ItemsGrid_Command(ByVal sender As Object, ByVal e As DataGridCommandEventArgs)
            ...
            End Select
        End Sub
        Sub itemGrid11(ByVal sender As Object, ByVal e As System.EventArgs)
            'myConn.ConnectionString = ConfigurationManager.AppSettings("ConnectionString1")
            Dim i As Integer
            Dim sql As String
            Dim a As Integer
            Dim dr As SqlDataReader
            myConnection.Open()
            For i = 0 To DataGrid1.Items.Count - 1
                Dim stcode As String = DataGrid1.Items(i).Cells(0).Text
                Dim checked As TextBox = CType(DataGrid1.Items(i).FindControl("dun"), TextBox)
                If  Mid(stcode, 1, 1) = "C" Then
                    sql = "select * from _AA8 where AH4='" & stcode.Trim & "' and AH2='" & Session("hichjil") & "'" & _
                    " and AH3=" & Session("hichses")
                    Dim cmd = New SqlCommand(sql, myConnection)
                    'Dim dr As SqlDataReader
                    dr = cmd.ExecuteReader()
                    If dr.Read() Then
                        a = CInt(IIf(Not IsDBNull(dr("AH17")), dr("AH17"), 0))
                    Else
                        a = 0
                    End If
                    dr.Close()
        ...
        Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
            ...
            For i = 0 To DataGrid2.Items.Count - 1
                ...
                'If val < dun Or 0 > dun Then
                '    'If val < CDbl(Replace(dun.Text, ",", ".")) Or 0 > CDbl(Replace(dun.Text, ",", ".")) Then
                '    Label6.Text = "Алдаа! " & hname.Text & "-н дvн буруу орсон байна."
                '    Exit Sub
                'End If
        ...
        Private Sub linkButton5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles linkButton5.Click
            Session("sname") = ChooseSchool.SelectedItem
            If ChooseSchool.SelectedValue = "1" Then
                'niit
                Response.Redirect("result3.aspx?al=" & ChooseSchool.SelectedValue & "&lesscode=" & Request("lesscode") & "&type=" & Request("type") & "&gr=" & Request("gr"))
            Else
                'surguuliar
                Response.Redirect("result1.aspx?al=" & ChooseSchool.SelectedValue & "&lesscode=" & Request("lesscode") & "&type=" & Request("type") & "&gr=" & Request("gr"))
            End If
        End Sub
